resource "aws_vpc" "main" {
  cidr_block = var.vpc_config.cidr_block
  tags = {
    Name = var.vpc_config.name
  }
}

resource "aws_subnet" "subnet_main" {
  vpc_id = aws_vpc.main.id

  //because subnet is a list so we will use for each
  for_each = var.subnet_config // key = {cidr_block,AZ} each.key and each.value

  cidr_block        = each.value.cidr_block
  availability_zone = each.value.az

  tags = {
    Name = each.key // because we have multiple subnet and people will be giving diff names
  }
}

locals {
  public_subnet = {
    //key={} if public is true in subnet_config
    for key, config in var.subnet_config : key => config if config.public
  }
  private_subnet = {
    #key={} if public is true in subnet_config
    for key, config in var.subnet_config : key => config if !config.public
  }
}

// Internet Gateway , if there is at least one public subnet
resource "aws_internet_gateway" "main_igw" {
  vpc_id = aws_vpc.main.id
  count  = length(local.public_subnet) > 0 ? 1 : 0
}

//Routing Table
resource "aws_route_table" "main_rt" {
  count  = length(local.public_subnet) > 0 ? 1 : 0
  vpc_id = aws_vpc.main.id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.main_igw[0].id
  }
}

//Route Table association
resource "aws_route_table_association" "main" {
  for_each = local.public_subnet #public_subnet={} private_subnet={}

  subnet_id      = aws_subnet.subnet_main[each.key].id
  route_table_id = aws_route_table.main_rt[0].id
}
